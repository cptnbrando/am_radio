...
// All of this is his, I just changed it a bit for Angular
// Parsing through that massive Spotify audio analytics data terrified me
...
export default class Sync {
    ...
    // volumeSmoothing: number = 100;
    // pingDelay: number = 2500;
    state: any;
    hooks: any;
    ...
    constructor(volumeSmoothing?, pingDelay?, private spotifyService?: SpotifyService, private playerService?: SpotifyPlayerService)
    {
      const accessToken = localStorage.get('accessToken');
      const refreshToken = localStorage.get('refreshToken');
      const refreshCode = localStorage.get('SPOTIFY_REFRESH_CODE');

      if(typeof volumeSmoothing !== 'undefined') this.volumeSmoothing = volumeSmoothing;
      if(typeof pingDelay !== 'undefined') this.pingDelay = pingDelay;
   ...
   */
  async getNewToken () {
    await this.spotifyService.getNewTokens().subscribe(data => {
      if(data) {
        localStorage.clear();
        localStorage.setItem("accessToken", data.message);

        this.state.api.tokens.accessToken = data.message;
      }
    })
  ...
  {
    await this.playerService.getCurrentlyPlaying().subscribe(data => {
  ...
  async getTrackInfo (data) {
    ...
    // Get Audio Analysis
    let analysis = {};
    await this.playerService.getAudioAnalysis(data.item.id).subscribe(data => {
      if(data) analysis = data;
    });

    // Get Audio Features
    let features = {};
    await this.playerService.getAudioFeatures(data.item.id).subscribe(data => {
  ...
  setActiveIntervals () {
    const determineInterval = (type) => {
      ...
      for (let i = 0; i < analysis.length; i++) {
        ...
        // This is the magic right here, wowee
  ...
  tick (now) {
    ...
    const sizeScale = scaleLog().domain([min(queues.volume), average(queues.volume)]).range([0, 1]);

    /** Average the beat queue, then pass it to our size scale. */
    const beat = average(queues.beat);
    this.state.volume = sizeScale(beat);
  }
};
